...
import { Question, Pagination, FilteredQuestionListConfig } from '@/services/apiTypes';
...
type QuestionContextAction =
  ...
  | { type: 'SET_FILTERS'; payload: { difficulty?: string | string[]; tags?: string[] } }
...
const reducer = (
  ...
  switch (action.type) {
    case 'SET_SUBJECT_ID':
      return { ...state, subjectId: action.payload, questions: [], pagination: initialState.pagination };
    case 'SET_FILTERS':
      return { ...state, filters: action.payload, questions: [], pagination: initialState.pagination };
    case 'SET_LOADING':
...
export const QuestionProvider = ({ children, initialSubjectId = '' }: QuestionProviderProps) => {
  // 使用初始主题ID更新初始状态
  ...
  const fetchData = useCallback(async (
    ...
    try {
      ...
      const response = await questionApiService.getFilteredQuestionList(config);

      if (response.success && response.data?.questions) {
        const questions = response.data.questions || [];
        const pagination = response.data.pagination || initialState.pagination;
  ...
  // 更新筛选条件
  const updateFilters = useCallback((filters: { difficulty?: string | string[]; tags?: string[] }) => {
