...
- 新增 users 集合，支持用户身份认证
- 更新 userActionController.js 中的 login 和 logout 函数，使用 users 集合进行认证
- 更新用户行为数据模型，支持用户登录状态管理
- 在示例代码中添加登录和退出登录的使用示例
- 在 API 文档中添加用户(User)数据模型定义
- 在 init-db.js 中添加 users 集合初始化和默认管理员用户创建
